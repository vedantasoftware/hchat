...
public class HChat {
	...
	public String toString() {
		return "serverIP=" + this.serverIP + "::port=" + this.port + "::userName=" + this.userName + "::password="
				+ this.password;
	}
	...
	public static void main(String[] args) throws IOException {
		...
		subscribeChatObj.hChat.clientId = computername + "HSubscribe";
		...
		publishFileTransferObj.SendBinaryFile("D:/Bala.mp4");

		/*
		 * HChat hChat = new HChat(); byte[] header =
		 * hChat.TestHeader("D:/Bala.jpg"); if( header != null) {
...
class FileHeader {
	...
	public String GetFileName() {
		...
		System.arraycopy(FileName, 0, fileName, 0, i);
	...
	public void SetFileName(String Filename) {
		...
		System.arraycopy(Filename.getBytes(), 0, FileName, 0, filenameLen);
	}

	public void SetFileSize(int size) {
		byte[] baFileSize = toByteArray(size);
		System.arraycopy(baFileSize, 0, FileSize, 0, 4);
	}

	/*
	 * private byte[] intToBytes(int value) { return new byte[] { (byte) ((value
	 * >>> 24) & 0xff), (byte) ((value >>> 16) & 0xff), (byte) ((value >>> 8) &
	...
	Boolean Parse(byte[] header) {
		if (header.length == 44) {
			...
			System.arraycopy(header, 0, BOFHeader, 0, 8);
			String BOFStr = new String(BOFHeader);
			if (BOFStr.compareTo(BOF) != 0)
				return false;

			System.arraycopy(header, 8, FileName, 0, 32);
			System.arraycopy(header, 40, FileSize, 0, 4);
